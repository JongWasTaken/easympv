...
function osdm(s)
  if s == "active" then
    mp.osd_message(mp.get_property_osd("osd-ass-cc/0") .. "Discord RPC: {\\c&H0cff00&}enabled" ..  mp.get_property_osd("osd-ass-cc/1"))
  elseif s == "inactive" then
		mp.osd_message(mp.get_property_osd("osd-ass-cc/0") .. "Discord RPC: {\\c&H3440eb&}disabled" ..  mp.get_property_osd("osd-ass-cc/1"))
  end
end

client_id = 536293699945234433LL
--client_id = 798647747678568488LL -- original
...
-- Keybinding
mp.add_key_binding(nil, "drpc_toggle", function ()
...
local function main()
  ...
  local file_path = mp.get_property("filename/no-ext") -- URL, or file path
  if file_path ~= nil then
    file_path = file_path:gsub('%b[]', '')
    file_path = file_path:gsub('%b()', '')
    file_path = file_path:gsub('%.mkv', '')
    file_path = file_path:gsub('%.mp4', '')
    file_path = file_path:gsub('%.', ' ')
    file_path = file_path:gsub('%_', ' ')
    file_path = file_path:gsub('%video=', '')
  end
