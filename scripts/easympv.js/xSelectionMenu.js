...
var SelectionMenu = function (settings) {
	...
	this.uniqueId =
		"M" +
		String(mp.get_time_ms()).replace(/\./g, "").substring(3) +
	...
	this.selectionIdx = 0;
	this.cbMenuShow =
		typeof settings.cbMenuShow === "function" ? settings.cbMenuShow : null;
	this.cbMenuHide =
		typeof settings.cbMenuHide === "function" ? settings.cbMenuHide : null;
	this.cbMenuLeft =
		typeof settings.cbMenuLeft === "function" ? settings.cbMenuLeft : null;
	this.cbMenuRight =
		typeof settings.cbMenuRight === "function"
			? settings.cbMenuRight
			: null;
	this.cbMenuOpen =
		typeof settings.cbMenuOpen === "function" ? settings.cbMenuOpen : null;
	this.cbMenuUndo =
		typeof settings.cbMenuUndo === "function" ? settings.cbMenuUndo : null;
	this.maxLines =
		typeof settings.maxLines === "number" && settings.maxLines >= 3
			? Math.floor(settings.maxLines)
			: 10;
	this.titleImage =
		typeof settings.titleImage === "string" ? settings.titleImage : null;
	this.titleImageX =
		typeof settings.titleImageX === "number" ? settings.titleImageX : null;
	this.titleImageY =
		typeof settings.titleImageY === "number" ? settings.titleImageY : null;
	this.menuFontAlpha = SSA.convertPercentToHex(
		// Throws if invalid input.
		typeof settings.menuFontAlpha === "number" &&
			settings.menuFontAlpha >= 0 &&
			settings.menuFontAlpha <= 1
			? settings.menuFontAlpha
			: 1,
		true // Invert input range so "1.0" is visible and "0.0" is invisible.
	);
	this.menuFontSize =
		typeof settings.menuFontSize === "number" && settings.menuFontSize >= 1
			? Math.floor(settings.menuFontSize)
			: 40;
	...
	this.autoCloseDelay =
		typeof settings.autoCloseDelay === "number" &&
	...
	this.keyBindings = {
		...
		"Menu-Left": { repeatable: false, keys: ["left"] },
		"Menu-Right": { repeatable: false, keys: ["right"] },
		"Menu-Open": { repeatable: false, keys: ["enter", "playpause"] },
		"Menu-Undo": { repeatable: false, keys: ["bs"] },
		"Menu-Help": { repeatable: false, keys: ["h"] },
		"Menu-Close": { repeatable: false, keys: ["esc", "m"] },
	};
	...
	if (rebinds) {
		for (action in rebinds) {
			if (!rebinds.hasOwnProperty(action)) continue;
	...
	for (action in this.keyBindings) {
		if (!this.keyBindings.hasOwnProperty(action)) continue;
		allKeys = this.keyBindings[action].keys;
		for (i = 0; i < allKeys.length; ++i) {
			key = allKeys[i];
			if (boundKeys.hasOwnProperty(key))
				throw (
					'Invalid duplicate menu bindings for key "' +
					key +
					'" (detected in action "' +
					action +
					'")'
...
SelectionMenu.prototype.setTitle = function (newTitle) {
	if (typeof newTitle !== "string") throw "setTitle: No title value provided";
	this.title = newTitle;
};

SelectionMenu.prototype.setDescription = function (newDescription) {
	if (typeof newDescription !== "string")
		throw "setDescription: No description value provided";
	this.description = newDescription;
};

SelectionMenu.prototype.setIdx = function (newIdx) {
	if (typeof newIdx !== "number") throw "newIdx: No value provided";
	this.selectionIdx = newIdx;
};

SelectionMenu.prototype.getIdx = function () {
	return this.selectionIdx;
};

SelectionMenu.prototype.setOptions = function (
	newOptions,
	initialSelectionIdx
) {
	...
	this.selectionIdx =
		typeof initialSelectionIdx === "number" &&
		initialSelectionIdx >= 0 &&
		initialSelectionIdx < newOptions.length
			? initialSelectionIdx
			: 0;
};
...
SelectionMenu.prototype.setUseTextColors = function (value) {
	...
	if (hasChanged) this.renderMenu(null, 1); // 1 = Only redraw if menu is onscreen.
};
...
SelectionMenu.prototype.getSelectedItem = function () {
	if (this.selectionIdx < 0 || this.selectionIdx >= this.options.length)
		return "";
	else return this.options[this.selectionIdx];
};
...
SelectionMenu.prototype._processBindings = function (fnCb) {
	...
	for (action in bindings) {
		if (!bindings.hasOwnProperty(action)) continue;
...
SelectionMenu.prototype._registerMenuKeys = function () {
	if (this.hasRegisteredKeys) return;
...
SelectionMenu.prototype._menuAction = function (action) {
	if (this.isShowingMessage && action !== "Menu-Close") return; // Block everything except "close" while showing a message.
	...
	switch (action) {
		...
		case "Menu-Down-Fast":
			...
			else this.selectionIdx += action === "Menu-Down-Fast" ? 10 : 1;
		...
		case "Menu-Help":
			...
			var entry,
				...
				helpLines = 0,
				helpString = SSA.startSeq(c) + SSA.alpha(this.menuFontAlpha, c),
				bindings = this.keyBindings;
			for (entry in bindings) {
				if (!bindings.hasOwnProperty(entry)) continue;
				allKeys = bindings[entry].keys;
				if (!entry.match(/^Menu-/) || !allKeys || !allKeys.length)
					continue;
				entryTitle = entry.substring(5);
				if (!entryTitle.length) continue;
				Utils.quickSort(allKeys, { caseInsensitive: true });
				++helpLines;
				helpString +=
					SSA.darkred(c) +
					SSA.esc(entryTitle, c) +
					": " +
					SSA.white(c) +
					SSA.esc("{" + allKeys.join("}, {") + "}", c) +
					"\n";
			}
			helpString += SSA.stopSeq(c);
			if (!helpLines) helpString = "No help available.";
...
SelectionMenu.prototype._updateAutoCloseTimeout = function (forceUnlock) {
	if (!forceUnlock && this.autoCloseActiveAt === -2) return; // Do nothing while autoclose is locked in "disabled" mode.

	this.autoCloseActiveAt = mp.get_time();
};

SelectionMenu.prototype._handleAutoClose = function () {
	if (this.autoCloseDelay <= 0 || this.autoCloseActiveAt <= -1)
		// -2 = hard, -1 = soft.
		return; // Do nothing while autoclose is disabled (0) or locked (< 0).

	var now = mp.get_time();
	if (this.autoCloseActiveAt <= now - this.autoCloseDelay) this.hideMenu();
};
...
SelectionMenu.prototype.renderMenu = function (
	...
	finalString =
		SSA.startSeq(c) +
		SSA.alpha(this.menuFontAlpha, c) +
		SSA.gray(c) +
		SSA.scale(80, c) +
		SSA.esc(this.title, c) +
		SSA.scale(100, c) +
		SSA.white(c) +
		"\n\n" +
		SSA.alpha(this.menuFontAlpha, c) +
	...
	if (this.options.length > 0) {
		...
		for (var i = startIdx; i <= endIdx; ++i) {
			opt = this.options[i];
			temp = SSA.esc(typeof opt === "object" ? opt.menuText : opt, c);
			if (i === this.selectionIdx)
				...
				finalString +=
					...
					(typeof selectionPrefix === "string"
						? SSA.esc(selectionPrefix, c) + " "
						: "");
			finalString +=
				typeof selectionSuffix === "string"
					? SSA.esc(selectionSuffix, c) + " "
					: "";
			if (typeof replacement === "string") {
				temp = SSA.esc(
					typeof replacement === "string" ? replacement : opt,
					c
				);
			...
			if (i === this.selectionIdx) finalString += SSA.white(c);
			if (i !== endIdx) finalString += "\n";
		}
	}
	// End the Advanced SubStation command sequence.
	finalString += SSA.stopSeq(c);
	...
	if (
		(renderMode === 1 && (!this.isMenuActive() || this.isShowingMessage)) ||
		renderMode === 2
	)
		return;
	this._showMenu();
...
SelectionMenu.prototype.hideMenu = function () {
	...
	if (typeof this.cbMenuHide === "function") this.cbMenuHide("Menu-Hide");
};

SelectionMenu.prototype.showMessage = function (
	msg,
	durationMs,
	...
	if (typeof msg !== "string") msg = "showMessage: Invalid message value.";
	if (typeof durationMs !== "number") durationMs = 800;

	if (clearSelectionPrefix) this.renderMenu(null, 2); // 2 = Only update text cache (no redraw).
	...
	var self = this;
	if (this.stopMessageTimeout !== null) clearTimeout(this.stopMessageTimeout);
...
SelectionMenu.prototype.stopMessage = function (preventRender) {
	...
	this.isShowingMessage = false;
	this.currentMessageText = "";
	if (!preventRender) this._renderActiveText();
