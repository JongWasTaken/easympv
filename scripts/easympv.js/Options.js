...
// TODO: reimplement this module to get out of Apache 2.0

var ScriptConfig = function (options, identifier) {
	if (!options) throw "Options table parameter is missing.";
	this.options = options;
	this.scriptName = typeof identifier === "string" ? identifier : "easympv";
	...
	if (this.scriptName && this.scriptName.length) {
		...
		this.configFile = mp.find_config_file(
			"script-opts/" + this.scriptName + ".conf"
		);
		if (!this.configFile)
	...
	if (this.configFile && this.configFile.length) {
		try {
			var line,
				configLines = mp.utils
					.read_file(this.configFile)
					.split(/[\r\n]+/);
...
ScriptConfig.prototype.setValue = function (key, value) {
	if (!this.options.hasOwnProperty(key)) {
		throw 'Invalid option "' + key + '"';
	}
	Utils.openFile(
		"easympvUtility.exe edit " + this.scriptname + " " + key + " " + value
	);
};
...
ScriptConfig.prototype.getMultiValue = function (key) {
	...
	if (len) {
		if (val.charAt(0) !== "{" || val.charAt(len - 1) !== "}")
			throw (
				'Missing surrounding "{}" brackets in multi-value option "' +
				key +
				'"'
			);
